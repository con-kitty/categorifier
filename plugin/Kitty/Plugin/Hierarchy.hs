...
module Kitty.Plugin.Hierarchy
  ( First (..),
    ...
    -- @baseMakers@ / `Kitty.Plugin.Core.PrimOp.replace`), we need to recognize these things.  The
    -- `Lookup` monad run outside the call to `Kitty.Plugin.Core.Categorize.categorize` is where we
...
concatOps = do
  ...
  ffcallH <- do
    op <- identifier "Kitty.Plugin.Category" "ffcall"
  ...
  indirectionH <- do
    op <- identifier "Kitty.Plugin.Category" "indirection"
  ...
  where
    repOp name = do
      op <- identifier "Kitty.Plugin.Core.Functions" name
...
--   applied).
--
--  __TODO__: We should perhaps have the fields return @`Kitty.Plugin.Core.Makers.CategoryStack`
...
baseHierarchy = do
  ...
  abstCV <-
    pure <$> do
      ...
      op <- identifier "Kitty.Plugin.Client" "abst"
      pure $ \onDict cat a ->
  ...
  distlV <-
    pure <$> do
      arr <- identifier "Control.Arrow" "arr"
      op <- identifier "Kitty.Plugin.Core.Base" "distlB"
  ...
  fixV <-
    pure <$> do
      fn <- identifier "Kitty.Plugin.Core.Base" "fixB"
  ...
  ifV <-
    pure <$> do
      arr <- identifier "Control.Arrow" "arr"
      op <- identifier "Kitty.Plugin.Core.Base" "ifThenElseB"
      pure $ \onDict cat a ->
        mkMethodApps
          onDict
          arr
  ...
  lassocV <-
    pure <$> do
      arr <- identifier "Control.Arrow" "arr"
      op <- identifier "Kitty.Plugin.Core.Base" "lassocB"
      pure $ \onDict cat a b c ->
        mkMethodApps
          onDict
          arr
  ...
  rassocV <-
    pure <$> do
      arr <- identifier "Control.Arrow" "arr"
      op <- identifier "Kitty.Plugin.Core.Base" "rassocB"
      pure $ \onDict cat a b c ->
        mkMethodApps
          onDict
          arr
  ...
  reprCV <-
    pure <$> do
      arr <- identifier "Control.Arrow" "arr"
      rep <- findTyCon "Kitty.Plugin.Client" "Rep"
      op <- identifier "Kitty.Plugin.Client" "repr"
      pure $ \onDict cat a ->
  ...
  strengthV <-
    pure <$> do
      arr <- identifier "Control.Arrow" "arr"
      op <- identifier "Kitty.Plugin.Core.Base" "strengthB"
  ...
  uncurryV <-
    pure <$> do
      fn <- identifier "Kitty.Plugin.Core.Base" "uncurryB"
