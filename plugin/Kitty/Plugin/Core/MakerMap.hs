...
module Kitty.Plugin.Core.MakerMap
  ( MakerMap,
    MakerMapFun,
    combineMakerMapFuns,
    baseMakerMapFun,
...
import Kitty.Duoidal (joinD, traverseD, (<*\>), (<=\<), (=<\<))
import qualified Kitty.Plugin.Core.Functions
import Kitty.Plugin.Core.Makers (Makers (..), extract2TypeArgs, getMorphismType)
import Kitty.Plugin.Core.Types (CategoricalFailure (..), CategoryStack)
import Kitty.Plugin.Hierarchy (properFunTy)
      ...
      makerMap =
        Map.fromListWith
          ...
          [ ( '(Control.Arrow.&&&),
            ...
            ( 'Kitty.Plugin.Core.Functions.abst,
              \case
                Plugins.Type a : _hasRep : rest -> pure $ maker1 rest =<\< mkAbstC a
                _ -> Nothing
            ),
            ( 'Kitty.Plugin.Core.Functions.repr,
              \case
