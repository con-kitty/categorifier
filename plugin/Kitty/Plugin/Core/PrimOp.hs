...
module Kitty.Plugin.Core.PrimOp
  ( -- * Replacement
    replace,
    Boxer (..),
...
import Kitty.Duoidal (sequenceD, traverseD, (<*\>))
import Kitty.Plugin.Core.Makers (Makers (..), isFreeIn)
import Kitty.Plugin.Core.Trace (addIdInfo, maybeTraceWith, renderSDoc)
import Kitty.Plugin.Core.Types (CategoricalFailure (..), CategoryStack)
...
-- | This contains all the data we need to get from `Kitty.Plugin.Core.categorizeLambda`.  It gets
...
--  `Kitty.Plugin.Core.categorizeLambda` that we use in replacement. The map lets us look up the
...
mkBoxedVarFrom pfx ty v = do
  ...
  where
    ...
    errString =
      "<Kitty.Plugin.Core.Categorize: unboxing `case` transformation var>"
